...
public class ApiRequester {
    ...
    public ArtPiece getSearchArtPiece(String etiqueta, Integer departmentId, boolean isHighlight) throws MalformedURLException {
        ...
        try{
            ...
            if (isHighlight) {
                searchUrl.append("&isHighlight=true");  //Si escogio buscar solo obras populares concatenamos el parámetro adecuado también
            }
            ResponseList response = mapper.readValue(new URL(searchUrl.toString()), ResponseList.class);    //Mapeamos la respuesta a nuestra clase ResponseList


            if (response.getObjectIDs() != null && !response.getObjectIDs().isEmpty()) {    //Comprobamos si encontramos objetos que se ajusten a los filtros de búsqueda
                int firstId = response.getObjectIDs().get(0);   //Leemos el primer id de la lista
                System.out.println(firstId);
                
                if (this.objectsCache.existsId(String.valueOf(firstId))) {  //Comprobamos si tenemos el id en la cache
                    ...
                    return artPiece;    //Devolvemos el objeto desde cache
                }
                
                URL artPieceUrl = new URL(baseURL.concat("/").concat(String.valueOf(firstId))); //Construimos una url de búsqueda por el id de la obra escogida
                
                JsonNode root = mapper.readTree(artPieceUrl);
                this.objectsCache.add(root.findValue("objectID").asText(), root.toString());    //Guardamos el objeto en cache con su id como clave
                this.objectsCache.save();

                artPiece = mapper.readValue(artPieceUrl, ArtPiece.class);   //Mapeamos la response a nuestra clase ArtPiece
                System.out.println(artPiece);
            }else{
                System.out.println("No se encontró ninguna obra para la etiqueta: " + etiqueta);
                return null;    //Si la búsqueda no resulto en ningún objeto devolvemos null
            }
        ...
        return artPiece;    //Devolvemos nuestro objeto ArtPiece para que el controlador pueda usarlo para actualizar la vista
